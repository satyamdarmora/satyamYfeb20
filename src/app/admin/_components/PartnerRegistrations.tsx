                        ...
                        <div style={{ padding: '10px 14px', whiteSpace: 'nowrap' }}>
                          {(reg.status === 'PENDING' || reg.status === 'INFO_REQUIRED') ? (
                            <div style={{ display: 'flex', gap: 6 }} onClick={(e) => e.stopPropagation()}>
                              <button onClick={() => onReview({ id: reg.id, regId: reg.registrationId, action: 'APPROVE' })} style={{ padding: '5px 12px', fontSize: 11, fontWeight: 600, background: 'var(--positive)', color: '#FFF', border: 'none', borderRadius: 5, cursor: 'pointer' }}>Approve</button>
                              ...
                              <button onClick={() => onReview({ id: reg.id, regId: reg.registrationId, action: 'INFO_REQUIRED' })} style={{ padding: '5px 12px', fontSize: 11, fontWeight: 600, background: 'transparent', color: 'var(--brand-primary)', border: '1px solid var(--brand-primary)', borderRadius: 5, cursor: 'pointer' }}>Info</button>
                            </div>
