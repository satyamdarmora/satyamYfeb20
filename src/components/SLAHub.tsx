...
export default function SLAHub({ onBack }: SLAHubProps) {
  ...
  useEffect(() => {
    ...
    fetchSLA();
    const interval = setInterval(fetchSLA, 3000);
    ...
    return (
      <div style={overlay}>
        <div style={{ padding: '16px', borderBottom: '1px solid var(--border-subtle)', flexShrink: 0 }}>
          ...
          <div style={{ fontSize: 18, fontWeight: 700, color: 'var(--text-primary)' }}>SLA Dashboard</div>
        </div>
        <div style={{ ...scroll, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
