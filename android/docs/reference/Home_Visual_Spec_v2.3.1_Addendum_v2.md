...
| DRILLDOWN_REQUIRED mode | §9: "Tap CTA → Optimistic UI" | No | New mode for CTAs requiring in-drilldown completion. v2.3 doesn't address this. New territory. |
| Time-limited card display | §5, §8 | No | v2.3 doesn't address card auto-dismissal. New pattern for terminal/resolved states. |
...
|---|---|---|
| DIRECT | Single tap → instant action. Optimistic UI. No confirmation. | Dispatch, Start Work, Call Customer |
| ACTION_SHEET | Tap → bottom sheet with structured input → confirm → action | Exit (needs reason), Schedule (needs dates), Assign (needs selection), Diagnose (needs structured input) |
| DRILLDOWN_REQUIRED | Tap → drilldown opens → CSP completes required steps → submit from drilldown | Resolve (needs proof before submission). Any action requiring in-context completion with multiple dependent steps. |
...
- DRILLDOWN_REQUIRED CTAs open the task drilldown. The drilldown contains the required steps (proof, checklist, multi-step input). Submit lives inside the drilldown, not on the card. The CTA on the card is the entry point, not the action itself.
...
## 4a. Time-Limited Card Display [Layer 2 extension]

Some task resolutions produce a brief confirmation before the card exits the queue. The CSP needs to see "done" before the card disappears — otherwise, silent disappearance could feel like a glitch.

### Pattern

| Property | Value |
|---|---|
| Trigger | Server sends card state with `auto_dismiss: true` + `dismiss_after_seconds: N` |
| Display | Card renders in resolved/confirmation state (✓ icon, positive styling) |
| Duration | Server-configured (typically 3-5 seconds) |
| Dismissal | Auto-removes from queue after duration. No CSP action needed. |
| Interaction | Card is NOT tappable during display. No CTA. No drilldown. Pure confirmation. |
| If CSP scrolls away | Card still auto-removes on timer. CSP doesn't need to watch it. |

### Rules

- Time-limited display is for terminal states only (RESOLVED, RECLASSIFIED, etc.)
- Card container, accent, and identity line unchanged — the CSP recognizes WHICH task resolved
- Reason line shows resolution confirmation text with `state.positive` styling
- No CTA. No interaction. This is not a passive card — it's a departing card.
- If server refresh arrives before auto-dismiss timer → card updates normally (server is authoritative)
...
|---|---|---|
| Execution modes — 3 modes (§1) | §9 Interaction Rules | No — refines "Tap CTA" |
...
| Time-limited card display (§4a) | §5, §8 | No — new pattern for terminal states |
...
| Batch threshold (§6) | §6 batch block (future) | No — documents future rule |

---

*Home Screen Visual Spec v2.3.1 (Addendum) | March 31, 2026*
*11 additions. 0 contradictions (1 purposeful upgrade of ●/○ → icons). Verified conflict-free against v2.3.*
